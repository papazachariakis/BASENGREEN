"""Home Assistant coordinator for Basen Green BMS integration."""
...
class BasenGreenCoordinator(DataUpdateCoordinator[BMSSample]):
    """Update coordinator for a Basen Green Tianpower BMS."""
    ...
    def __init__(
        ...
        self._link_q: deque[bool] = deque([False], maxlen=100)
        self._mac: Final = ble_device.address
        self._stale: bool = False

        LOGGER.debug(
            "Initializing Basen Green coordinator for %s (%s)",
            self.name,
            self._mac,
        )
    ...
    def link_quality(self) -> int:
        """Percentage of successful BMS reads out of the last 100 attempts."""
